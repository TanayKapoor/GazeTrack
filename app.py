...
st.write(f"Running on: {device}")  # Quick debug

# Load model
model = AutoModelForCausalLM.from_pretrained(
    "vikhyatk/moondream2",  # Public model, no auth needed
    trust_remote_code=True,
    torch_dtype=torch.float32,  # Safe for MPS
    device_map=device,
    revision="main"  # Latest stable
)

# Visualization function from demo.py 
...
# Process image with moondream2 gaze detection
def process_image(image, ensemble=False):
    try:
        # Get image encoding
        ...
        for face in faces:
            ...
            face_box = (
                face["x_min"] * image.width,
                face["y_min"] * image.height,
                (face["x_max"] - face["x_min"]) * image.width,
                (face["y_max"] - face["y_min"]) * image.height,
            )
            face_center = (
        ...
        fig = visualize_faces_and_gaze(face_boxes, gaze_points, image=image_array, show_plot=False)
...
st.title('Tweaked Gaze Demo')
st.markdown("Powered by moondream/gaze-demo (CC-BY-SA weights)—shoutout vikhyat! [Original](https://huggingface.co/spaces/moondream/gaze-demo)")

image = st.file_uploader("Upload image", type=["jpg", "png"])
ensemble = st.checkbox("Use ensemble (flips for accuracy)")

if st.button("Detect Gaze") and image:
    pil_image = Image.open(image)
    fig, msg = process_image(pil_image, ensemble)
    if fig:
        st.pyplot(fig)
    st.write(msg)

# Bonus: Add your mod, like line color picker
line_color = st.color_picker("Gaze line color", "#FF0000")